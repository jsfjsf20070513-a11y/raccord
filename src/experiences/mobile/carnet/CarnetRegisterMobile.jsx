...
export default function CarnetRegisterMobile({ traces, contributions, loading, status }) {
  return (
    <article className="carnet-register carnet-register-mobile is-readonly">
      <header><p>Annexe · Le Carnet</p><h1>Provenance</h1><span>来源明确，身份分开</span></header>
      <section className="register-ledger" aria-label="可核验技术记录">
        <p className="register-section-label">Trace vérifiée · 技术记录</p>
        <ol>{traces.map((entry, index) => <li key={entry.id}><small>N°{String(index + 1).padStart(2, '0')}</small><blockquote>{entry.content}</blockquote><p>— {entry.signature} · {entry.date}</p><i>Solana devnet · provenance vérifiée</i><p className="register-source-links"><a {...externalLinkProps(entry.provenance.accountUrl)}>Account</a><a {...externalLinkProps(entry.provenance.transactionUrl)}>Transaction</a></p></li>)}</ol>
      </section>
