...
- **No real class photos in this public repo**: SVG placeholders only. Real class photos, admin SQL, and the production `.env` live outside this repository and are injected into the production build at deploy time via `scripts/prepare-private-assets.mjs` (gated by the `MATHCLASS_PRIVATE_REPO` env var). The repository itself never carries class photo binaries or any commit history of them. Real admin SQL files (`setup_admin*.sql`) are listed in `.gitignore` and never enter this public repo.
- **Stored-XSS defense in depth**: every user-visible external link in [`src/pages/Resources.jsx`](src/pages/Resources.jsx) and [`src/pages/ResourceDetail.jsx`](src/pages/ResourceDetail.jsx) goes through [`sanitizeHttpUrl`](src/lib/safeUrl.js), which rejects any protocol other than `http:` / `https:` / `mailto:` and falls back to `#`. Combined with `target="_blank" rel="noopener noreferrer"`, this blocks `javascript:` / `data:` / `vbscript:` payloads even if a contributor's draft were ever published unreviewed.
- **The Spring 2026 schedule keeps its real structure, with names anonymized**: `src/data/siteContent.js` preserves the actual timetable's classroom codes, time slots, and course cadence — that fidelity is the credibility signal for an "honest student builder" submission — but teacher names are replaced with neutral placeholders (e.g. `法方专业教师 M`). The real names live only in the private `MathClassWebsite` repo and are not committed here.
