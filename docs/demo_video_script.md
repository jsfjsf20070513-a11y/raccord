...
| 1:02–1:18 | Back to §03. Click the secondary button **"Try `class_anchor` on /witness →"** — page navigates to /witness. Scroll to /witness §02, click **"Anchor on devnet · 上链记录"**. Phantom popup → confirm. Show the resulting tx Solscan link + the PDA in /witness §03 history. | "Now the second anchor path — my custom Rust Anchor program `class_anchor`, written and deployed to devnet for this hackathon, program ID `Cmv8…BC4fu`. Source lives in `programs/class-anchor/src/lib.rs`. The /witness page is its dedicated demo: connect, write, and read PDAs back via `program.account.classAnchor.all`." |
